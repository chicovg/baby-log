...
export const toSummaryDisplayDate = dateKey => moment(dateKey).format(SUMMARY_DATE_DISPLAY_FORMAT);

export const toDateTime = (date, time) => moment(`${date} ${time}`, DATETIME_KEY_FORMAT);
...
export const toStoredTime = date => time =>
    moment(`${date} ${time}`, DATETIME_DISPLAY_FORMAT).format(TIME_KEY_FORMAT);

export const currentDateKey = () => moment().format(DATE_KEY_FORMAT);

export const previousDateKey = date =>
    moment(date)
        .subtract(1, 'days')
        .format(DATE_KEY_FORMAT);

export const nextDateKey = date =>
    moment(date)
        .add(1, 'days')
        .format(DATE_KEY_FORMAT);
